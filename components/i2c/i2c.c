/*
 * This is the I2C driver from the ESP-IDF.
 * Additions made in order to support our hardware slave emulation have been marked in a comment with the keyword ADDITION.
 *
...
#define I2C_FIFO_FULL_THRESH_VAL       (1)  /* ADDITION: set threshold to 1 byte, so that interrupt fires after every byte received*/
#define I2C_FIFO_EMPTY_THRESH_VAL      (2)
...
typedef struct {
    ...
    int slave_em_en;                 /*!< ADDITION: indicates if slave emulator mode is enabled */
    int slave_em_thresh;             /*!< ADDITION: if slave is used as emulator, this is the threshold for read bytes at a time */
    uint16_t slave_em_ints;          /*!< ADDITION: if slave is used as emulator, this is the interrupt disable mask used to disable interrupts */
    void (* slave_em_handler) (uint8_t *, int *, uint8_t *, int *, uint16_t); /*!< ADDITION: if slave is used as emulator, this is the pointer to function called in isr */
} i2c_obj_t;
...
esp_err_t i2c_slave_em_en(i2c_port_t i2c_num, int thresh, void (* handler) (uint8_t *, int *, uint8_t *, int *, uint16_t), uint16_t ints) {
    ...
    // ADDITION: Set I2C slave emulator mode flag to true.
    p_i2c_obj[i2c_num]->slave_em_en = 1;
    // ADDITION: Set threshold of bytes to receive to given parameter.
    p_i2c_obj[i2c_num]->slave_em_thresh = thresh;
    // ADDITION: Register custom handler for slave device.
    p_i2c_obj[i2c_num]->slave_em_handler = handler;
    // ADDITION: Set interrupt mask for I2C hardware module.
    p_i2c_obj[i2c_num]->slave_em_ints = ints;

    I2C_ENTER_CRITICAL(&(i2c_context[i2c_num].spinlock));
    i2c_hal_disable_intr_mask(&(i2c_context[i2c_num].hal), ints);
    I2C_EXIT_CRITICAL(&(i2c_context[i2c_num].spinlock));
...
static void IRAM_ATTR i2c_isr_handler_default(void *arg)
{
    ...
    if (p_i2c->mode == I2C_MODE_MASTER) {
        ...
        } else { /* ADDITION: This branch gets called, if I2C slave_em mode is enabled, to emulate an I2C peripheral.
                  * The received data is passed on to the slave handler function along with buffers that can store an answer.
                  * Afterwards, if an answer was written to the provided buffers, that answer is sent back to the I2C master.
                  */
            if(evt_type == I2C_INTR_EVENT_TRANS_DONE || evt_type == I2C_INTR_EVENT_RXFIFO_FULL) {
                uint32_t rx_fifo_cnt;
                i2c_hal_get_rxfifo_cnt(&(i2c_context[i2c_num].hal), &rx_fifo_cnt);
                i2c_hal_read_rxfifo(&(i2c_context[i2c_num].hal), &p_i2c->data_buf[p_i2c->rx_cnt], rx_fifo_cnt);
                p_i2c->rx_cnt += rx_fifo_cnt;
                if(p_i2c->rx_cnt >= p_i2c->slave_em_thresh || evt_type == I2C_INTR_EVENT_TRANS_DONE) {
                    uint8_t ans[32] = { 0 };
                    int ans_size = 0;
                    p_i2c->slave_em_handler(p_i2c->data_buf, &p_i2c->rx_cnt, ans, &ans_size, evt_type);
                    i2c_hal_txfifo_rst(&(i2c_context[i2c_num].hal));
                    i2c_hal_write_txfifo(&(i2c_context[i2c_num].hal), ans, ans_size);
                }
                if (evt_type == I2C_INTR_EVENT_TRANS_DONE) i2c_hal_clr_intsts_mask(&(i2c_context[i2c_num].hal), 0x80);
                if (evt_type == I2C_INTR_EVENT_RXFIFO_FULL) i2c_hal_clr_intsts_mask(&(i2c_context[i2c_num].hal), 0x1);
            }
        }
    }
